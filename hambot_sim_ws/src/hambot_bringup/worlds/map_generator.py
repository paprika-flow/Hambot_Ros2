...
class SidewalkMapGenerator:
    def __init__(self, root):
        ...
        self.drag_mode = None  # "draw", "move", "resize_length", "resize_width", "move_grass", "resize_grass_corner"
        ...
        self.active_corner_keys = None # Tuple of keys (x_key, y_key)
    ...
    def setup_ui(self):
        # 1. Left Sidebar (Scrollable Container)
        ...
        # Smart mousewheel scroll bindings
        ...
        # --------------------------------------------------
        self.pane_global = CollapsiblePane(self.sidebar.scrollable_frame, text="Global Settings")
        self.pane_global.pack(fill=tk.X, expand=True)
        global_content = self.pane_global.content

        # Active Tool Selector
        ttk.Label(global_content, text="Active Drawing Tool:", font=("Arial", 10, "bold")).pack(anchor=tk.W)
        ...
        ttk.Radiobutton(tool_frame, text="Sidewalk", variable=self.tool_var, value="sidewalk").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(tool_frame, text="Grass Area", variable=self.tool_var, value="grass").pack(side=tk.LEFT, padx=5)

        self.snap_var = tk.BooleanVar(value=True)
        self.snap_check = ttk.Checkbutton(global_content, text="Snap to 1m Grid", variable=self.snap_var)
        self.snap_check.pack(anchor=tk.W, pady=5)
        ...
        ttk.Label(global_content, text="Zoom Controls:").pack(anchor=tk.W)
        ...
        ttk.Button(zoom_frame, text="Zoom Out (-)", command=self.zoom_out).pack(side=tk.RIGHT, expand=True, fill=tk.X, padx=2)

        # --------------------------------------------------
        # PANEL 2: Precise Placement & Editing
        # --------------------------------------------------
        self.pane_edit = CollapsiblePane(self.sidebar.scrollable_frame, text="Precise Editor")
        self.pane_edit.pack(fill=tk.X, expand=True)
        self.edit_content = self.pane_edit.content

        self.sel_status_label = ttk.Label(self.edit_content, text="No segment selected", font=("Arial", 10, "italic"))
        self.sel_status_label.pack(anchor=tk.W, pady=(0, 10))

        # Precision Input Widgets (Packed dynamically in update_editor_labels)
        ...
        # Actions
        self.btn_add_new = ttk.Button(self.edit_content, text="Add as New Segment", command=self.add_precise_segment)
        self.btn_update = ttk.Button(self.edit_content, text="Apply Changes to Selected", command=self.update_selected_segment)
        self.btn_delete = ttk.Button(self.edit_content, text="Delete Selected", command=self.delete_selected_segment)
        self.btn_deselect = ttk.Button(self.edit_content, text="Deselect", command=self.deselect_segment)

        # --------------------------------------------------
        # PANEL 3: Map Actions & Export
        # --------------------------------------------------
        self.pane_actions = CollapsiblePane(self.sidebar.scrollable_frame, text="Map Actions & Export")
        self.pane_actions.pack(fill=tk.X, expand=True)
        actions_content = self.pane_actions.content
        
        ttk.Button(actions_content, text="Undo Last", command=self.undo_last).pack(fill=tk.X, pady=3)
        ttk.Button(actions_content, text="Clear Map", command=self.clear_map).pack(fill=tk.X, pady=3)
        ttk.Button(actions_content, text="Export SDF World", command=self.export_sdf).pack(fill=tk.X, pady=(15, 3))

        guide_text = (
            "Interactive Gestures:\n"
            "- Drag Segment Body: Translates its position.\n"
            "- Drag Blue Square: Resizes length & angle.\n"
            "- Drag Pink Square: Resizes width.\n"
            "- Drag Cyan Square: Resizes grass corners.\n"
            "- Red dots show starting anchors."
        )
        ttk.Label(actions_content, text=guide_text, justify=tk.LEFT, foreground="gray").pack(anchor=tk.W, pady=10)

    # --------------------------------------------------
    # UI Adaptive Display Formatting
    # --------------------------------------------------
    def on_tool_change(self):
        # Deselect segments that do not match the active tool type
    ...
    def update_editor_labels(self):
        """Restructuring panel inputs dynamically depending on active tool mode"""
    ...
    def update_button_states(self):
        if self.selected_idx != -1:
            ...
            self.sel_status_label.config(text=f"Editing Segment {self.selected_idx}", foreground="#ffa500")
        else:
    ...
    def find_clicked_segment(self, wx, wy):
        ...
        for idx, seg in enumerate(self.segments):
            ...
            # Mutual Exclusion Filter: Click hits ONLY match the active drawing tool
    ...
    # --------------------------------------------------
    # Mouse Interaction, Dragging & Resizing Engine
    # --------------------------------------------------
    def on_mouse_press(self, event):
        ...
        if self.selected_idx != -1:
            ...
            # Security filter: Ensure the selection belongs to the active tool
            if seg_type == active_tool:
                if seg_type == 'sidewalk':
                    x_s, y_s = seg['x'], seg['y']
                    L, yaw, w = seg['length'], seg['yaw'], seg['width']
                    ...
                    # Check 4 corner handle regions (Cyan squares)
                    corners = [
                        (x1, y1, "x1", "y1"),
                        (x2, y1, "x2", "y1"),
                        (x1, y2, "x1", "y2"),
                    ...
                    for cx_w, cy_w, x_key, y_key in corners:
                        ...
                        dist = math.sqrt((event.x - ccy)**2 + (event.y - ccx)**2)
        ...
        # 2. Check if clicked on any other segment matching active tool to select it
        ...
        # 3. Clicking empty space: Deselect and initiate draw tools
        self.selected_idx = -1
        self.update_button_states()
        self.update_editor_labels()
        self.redraw()
    ...
    def on_mouse_drag(self, event):
        ...
        move_dist = math.sqrt((event.x - self.press_cx)**2 + (event.y - self.press_cy)**2)
        if move_dist > 5:
    ...
    def redraw(self):
        ...
        # Strictly structured rendering stack:
        self.draw_grass_patches()    # Layer 1 (Bottom)
        self.draw_grid()             # Layer 2 (Middle - renders over grass)
        self.draw_sidewalks()        # Layer 3 (Top - renders over grass and grid)
        self.draw_active_handles()   # Layer 4 (Overlay - handles render on top of all)

    def draw_grid(self):
        for x in range(-50, 51):
            cx, cy_start = self.to_canvas(x, -50)
            _, cy_end = self.to_canvas(x, 50)
            color = "#404040" if x == 0 else "#363636"
            dash = () if x == 0 or x % 5 == 0 else (1, 5)
            self.canvas.create_line(cx, cy_start, cx, cy_end, fill=color, dash=dash, tags="grid")

        for y in range(-50, 51):
            cx_start, cy = self.to_canvas(-50, y)
            cx_end, _ = self.to_canvas(50, y)
            color = "#404040" if y == 0 else "#363636"
            dash = () if y == 0 or y % 5 == 0 else (1, 5)
            self.canvas.create_line(cx_start, cy, cx_end, cy, fill=color, dash=dash, tags="grid")
    ...
    def draw_grass_patches(self):
        ...
        for idx, seg in enumerate(self.segments):
            ...
            if idx == self.selected_idx:
                self.canvas.create_rectangle(min(cx1, cx2)-3, min(cy1, cy2)-3, max(cx1, cx2)+3, max(cy1, cy2)+3, 
                                             outline="#ffd700", width=3, tags="grass")
            
            # Render solid deep green block
            self.canvas.create_rectangle(cx1, cy1, cx2, cy2, 
                                         fill="#264d26", outline="#3b6b3b", width=1, tags="grass")
    ...
    def draw_sidewalks(self):
        ...
        for idx, seg in enumerate(self.segments):
            ...
            self.canvas.create_oval(cx1-4, cy1-4, cx1+4, cy1+4, 
                                    fill=anchor_color, outline="white", tags="sidewalk")

    def draw_active_handles(self):
        """Layer 4: Selection handles and text overlays render on top of all physics elements"""
        for idx, seg in enumerate(self.segments):
            seg_type = seg.get('type', 'sidewalk')
            ...
            if seg_type == 'sidewalk':
                ...
                if idx == self.selected_idx:
                    ...
                    self.canvas.create_rectangle(cx2-6, cy2-6, cx2+6, cy2+6, 
                                                 fill="#00bfff", outline="white", tags="overlay")
                    # Pink width handle
                    x_m = x_s + (L/2) * math.cos(yaw)
                    y_m = y_s + (L/2) * math.sin(yaw)
                    p_x = -math.sin(yaw)
                    ...
                    y_w = y_m + (w/2) * p_y
                    cx_w, cy_w = self.to_canvas(x_w, y_w)
                    self.canvas.create_rectangle(cx_w-6, cy_w-6, cx_w+6, cy_w+6, 
                                                 fill="#ff1493", outline="white", tags="overlay")
                ...
                self.canvas.create_text(mcx, mcy, text=f"S{idx}\n{L:.1f}m\nW:{w:.1f}m\nH:{h:.2f}m", 
                                        fill=text_color, font=("Arial", 8, "bold"), justify=tk.CENTER, tags="overlay")
                ...
                if idx == self.selected_idx:
                    ...
                    for px, py in [(x1, y1), (x2, y1), (x1, y2), (x2, y2)]:
                        ...
                        self.canvas.create_rectangle(ccx-5, ccy-5, ccx+5, ccy+5, 
                                                     fill="#00ffff", outline="white", tags="overlay")
                ...
                self.canvas.create_text(mcx, mcy, text=f"Grass G{idx}\n{abs(x2-x1):.1f}x{abs(y2-y1):.1f}m", 
                                        fill=text_color, font=("Arial", 8, "bold"), justify=tk.CENTER, tags="overlay")
    ...
    def clear_map(self):
        if messagebox.askyesno("Confirm Clear", "Are you sure you want to delete all segments?"):
            self.segments = []
            self.selected_idx = -1
            self.update_button_states()
            self.update_editor_labels()
    ...
    def export_sdf(self):
        ...
        # Separate custom grass assets from sidewalk geometry
        grass_patches = [s for s in self.segments if s.get('type', 'sidewalk') == 'grass']
        sidewalks = [s for s in self.segments if s.get('type', 'sidewalk') == 'sidewalk']

        # 1. GENERATE GRASS LAYOUTS
        if not grass_patches:
            # Fallback layout: Standard single giant grass plane centered on sidewalks
            if not sidewalks:
                min_x, max_x, min_y, max_y = -10, 10, -10, 10
            else:
                min_x = min(min(seg['x'], seg['x'] + seg['length']*math.cos(seg['yaw'])) for seg in sidewalks)
                max_x = max(max(seg['x'], seg['x'] + seg['length']*math.cos(seg['yaw'])) for seg in sidewalks)
                min_y = min(min(seg['y'], seg['y'] + seg['length']*math.sin(seg['yaw'])) for seg in sidewalks)
                max_y = max(max(seg['y'], seg['y'] + seg['length']*math.sin(seg['yaw'])) for seg in sidewalks)
        ...
        <visual name="visual">
          ...
          <plugin filename="ignition-gazebo-label-system" name="ignition::gazebo::systems::Label">
            ...
            for idx, gp in enumerate(grass_patches):
                ...
                dx = abs(x2 - x1)
                dy = abs(y2 - y1)
                H_grass = 0.1 # Grass thickness
                
                # Grass extends downwards from Z = 0.0
                line = f"""
...
{grass_network_xml}
        ...
        for idx, seg in enumerate(sidewalks):
            ...
            # Anchor bottom face at Z = 0.0. Offset visual/collision by +H/2
            line = f"""
